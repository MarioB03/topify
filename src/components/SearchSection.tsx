...
import { validateSearchQuery, validateAndSanitizeSong, generateSafeId } from '../utils/validation';
...
const SearchSection: React.FC = () => {
  ...
  const searchSongs = useCallback(async (query: string) => {
    // Si la búsqueda está vacía o es muy corta, limpiar resultados sin error
    if (query.length < 2) {
      dispatch({ type: actionTypes.SET_SEARCH_RESULTS, payload: [] });
      return;
    }

    // Validar y sanitizar la búsqueda solo si tiene contenido
    const validatedQuery = validateSearchQuery(query);
    
    if (!validatedQuery) {
      dispatch({ type: actionTypes.SET_SEARCH_RESULTS, payload: [] });
      dispatch({ type: actionTypes.SHOW_TOAST, payload: {
        message: 'Búsqueda inválida. Por favor usa solo texto.',
        type: 'error'
      }});
      return;
    }
    ...
    try {
      const lastfmResponse = await fetch(
        `https://ws.audioscrobbler.com/2.0/?method=track.search&track=${encodeURIComponent(validatedQuery)}&api_key=${process.env.NEXT_PUBLIC_LASTFM_API_KEY}&format=json&limit=5`
      );
      ...
      if (lastfmResponse.ok) {
        ...
        if (lastfmData.results && lastfmData.results.trackmatches && lastfmData.results.trackmatches.track) {
          ...
          const processedTracks = await Promise.all(tracks.map(async (track: { artist: string; name: string; image?: Array<{ '#text': string }> }) => {
            const trackData = {
              id: generateSafeId(track.artist, track.name),
              title: track.name,
              artist: {
                name: track.artist
              },
          ...
          // Validar y filtrar canciones
          const validatedTracks = processedTracks
            .map(track => validateAndSanitizeSong(track))
            .filter((track): track is NonNullable<typeof track> => track !== null);
          
          dispatch({ type: actionTypes.SET_SEARCH_RESULTS, payload: validatedTracks });
        } else {
          dispatch({ type: actionTypes.SET_SEARCH_RESULTS, payload: [] });
        }
      }
      ...
      dispatch({ type: actionTypes.SHOW_TOAST, payload: {
        message: 'Error al buscar canciones. Por favor intenta de nuevo.',
        type: 'error'
      }});
      dispatch({ type: actionTypes.SET_SEARCH_RESULTS, payload: [] });
    } finally {
