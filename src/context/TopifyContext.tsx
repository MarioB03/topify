...
import { getDeviceTracker, DeviceTracker } from '../utils/deviceTracking';
...
const deviceTracker = typeof window !== 'undefined' ? getDeviceTracker() : null;

// Initial state
const initialState: TopifyState = {
  playlist: isLocalhost ? mockPlaylist : [],
  votedSongs: isLocalhost ? mockVotedSongs : (deviceTracker?.getVotedSongs() || []),
  searchResults: [],
  currentPage: 0,
  myVotesPage: 0,
  isSearching: false,
  ...
  deviceId: deviceTracker?.getDeviceId() || '',
  isPrivateMode: deviceTracker?.isInPrivateMode() || false,
  stats: {
...
type Action = 
  ...
  | { type: 'SET_VOTED_SONGS'; payload: string[] }
  | { type: 'SET_SEARCH_RESULTS'; payload: Song[] }
  | { type: 'SET_CURRENT_PAGE'; payload: number }
  | { type: 'SET_MY_VOTES_PAGE'; payload: number }
  | { type: 'SET_IS_SEARCHING'; payload: boolean }
  | { type: 'SET_IS_LOADING_PLAYLIST'; payload: boolean }
  | { type: 'SET_IS_LOADING_VOTES'; payload: boolean }
  | { type: 'SET_CURRENT_AUDIO'; payload: { audio: HTMLAudioElement | null; id: string | null } }
  | { type: 'ADD_SONG_TO_PLAYLIST'; payload: Song }
  | { type: 'UPDATE_SONG_VOTES'; payload: { id: string; votes: number } }
  | { type: 'REMOVE_SONG_FROM_PLAYLIST'; payload: string }
  | { type: 'ADD_VOTED_SONG'; payload: string }
  | { type: 'REMOVE_VOTED_SONG'; payload: string }
  | { type: 'UPDATE_STATS'; payload?: undefined }
...
const topifyReducer = (state: TopifyState, action: Action): TopifyState => {
  switch (action.type) {
    ...
    case actionTypes.SHOW_TOAST:
      // Este será manejado por el componente Toast
      return state;
    default:
      return state;
  }
...
interface TopifyContextType {
  ...
  firestoreUtils: any;
  isReady: boolean;
  deviceTracker: DeviceTracker | null;
