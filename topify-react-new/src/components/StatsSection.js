    ...
    <section className={`glass-card rounded-3xl ${isMobile ? 'p-4' : 'p-6'} flex-shrink-0`}>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div 
          ...
          style={{'--from-color': 'var(--color-sage-medium)', '--to-color': 'var(--color-sage-dark)'}}
        >
          <div className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold mb-1`}>
            {state.stats.totalSongs}
          </div>
          <div className="text-xs font-medium opacity-90">Canciones</div>
        </div>
        
        <div 
          className="stat-card text-white p-4 rounded-2xl text-center relative overflow-hidden" 
          style={{'--from-color': 'var(--color-gold-medium)', '--to-color': 'var(--color-gold-dark)'}}
        >
          <div className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold mb-1`}>
            {state.stats.totalVotes}
          </div>
          <div className="text-xs font-medium opacity-90">Votos totales</div>
        </div>
      </div>
      
      <div 
        className="stat-card text-white p-4 rounded-2xl text-center relative overflow-hidden" 
        style={{'--from-color': 'var(--color-rose-medium)', '--to-color': 'var(--color-rose-dark)'}}
      >
